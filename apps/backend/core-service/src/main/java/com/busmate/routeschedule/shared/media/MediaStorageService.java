...
public class MediaStorageService {
    ...
    public Optional<StoredObject> get(String key) {
        try {
            ...
            // A record can reference a key whose object is gone — a restore that recovered the
            // database but not the bucket, for instance. That is missing media, not a failure.
    ...
    public void delete(String key) {
        mediaS3Client.deleteObject(DeleteObjectRequest.builder().bucket(bucket).key(key).build());
    }
}
