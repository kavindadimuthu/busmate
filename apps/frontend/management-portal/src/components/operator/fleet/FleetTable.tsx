...
const STATUS_META: Record<string, { label: string; icon: React.ReactNode; classes: string }> = {
  ...
  INACTIVE:    { label: 'Inactive',    icon: <XCircle     className="w-3.5 h-3.5" />, classes: 'bg-warning/15 text-warning border-orange-200' },
  MAINTENANCE: { label: 'Maintenance', icon: <Wrench      className="w-3.5 h-3.5" />, classes: 'bg-warning/15 text-warning border-warning/20' },
  RETIRED:     { label: 'Retired',     icon: <AlertCircle className="w-3.5 h-3.5" />, classes: 'bg-muted text-muted-foreground border-border'       },
};
...
export function FleetTable({
  ...
  const columns: DataTableColumn<OperatorBus>[] = [
    ...
    },
    {
      ...
      render: (bus) => (
        <div>
          <div className="text-foreground/80">{bus.model}</div>
          <div className="text-xs text-muted-foreground/70">{bus.manufacturer}</div>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      header:   'Year',
      sortable: true,
      render: (bus) => <span className="text-muted-foreground">{bus.year}</span>,
    },
    {
      key:      'seatingCapacity',
      header:   'Seats',
      sortable: true,
      render: (bus) => <span className="text-muted-foreground">{bus.seatingCapacity}</span>,
    },
    {
      key:      'status',
      header:   'Status',
      ...
      render: (bus) =>
        bus.driver ? (
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-foreground/80 truncate max-w-[130px]">{bus.driver.driverName}</span>
          </div>
        ) : (
          <span className="text-muted-foreground/70 italic text-xs">Unassigned</span>
        ),
    },
    {
      key:    'route',
      ...
      render: (bus) =>
        bus.routeAssignments[0] ? (
          ...
          <span className="text-muted-foreground/70 italic text-xs">No route</span>
        ),
    },
    {
      key:             'actions',
