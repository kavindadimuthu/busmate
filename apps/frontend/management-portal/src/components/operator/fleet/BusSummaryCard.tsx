...
const STATUS_META: Record<string, { label: string; classes: string; icon: React.ReactNode }> = {
  ...
  INACTIVE:    { label: 'Inactive',    classes: 'bg-warning/15 text-warning border-orange-200', icon: <XCircle className="w-4 h-4" /> },
  MAINTENANCE: { label: 'Maintenance', classes: 'bg-warning/15 text-warning border-warning/20', icon: <Wrench className="w-4 h-4" /> },
  RETIRED:     { label: 'Retired',     classes: 'bg-muted text-muted-foreground border-border',       icon: <AlertCircle className="w-4 h-4" /> },
};
...
function ExpiryValue({ date }: { date: string }) {
  ...
  const cls = expired ? 'text-destructive font-semibold' : soon ? 'text-warning font-semibold' : 'text-foreground';
      ...
      <div className="bg-card rounded-xl border border-border shadow-sm p-6">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-foreground">{bus.plateNumber}</h1>
              <p className="text-sm text-muted-foreground font-mono mt-0.5">{bus.ntcRegistrationNumber}</p>
              <p className="text-sm text-muted-foreground mt-1">{bus.manufacturer} {bus.model} · {bus.year}</p>
            </div>
          </div>
        ...
        {/* Quick stats row */}
        <div className="mt-5 pt-5 border-t border-border/50 grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { icon: <Gauge className="w-4 h-4 text-purple-500" />, label: 'Seating', value: `${bus.seatingCapacity} seats` },
            { icon: <Calendar className="w-4 h-4 text-primary/80" />, label: 'Year',    value: bus.year },
            { icon: <Gauge className="w-4 h-4 text-warning/80" />,  label: 'Mileage', value: `${bus.mileage.toLocaleString()} km` },
            { icon: <Shield className="w-4 h-4 text-success/80" />,  label: 'Type',    value: SERVICE_TYPE_LABELS[bus.serviceType] ?? bus.serviceType },
          ].map(item => (
          ...
          {bus.driver ? (
            <>
              ...
              <InfoRow
                ...
                value={
                  <a href={`tel:${bus.driver.contactPhone}`} className="text-primary hover:underline flex items-center gap-1">
            ...
            <p className="text-sm text-muted-foreground/70 italic py-2">No driver assigned</p>
          )}
          ...
          {bus.conductor && (
            <>
              ...
              <InfoRow
                ...
                value={
                  <a href={`tel:${bus.conductor.contactPhone}`} className="text-primary hover:underline flex items-center gap-1">
        ...
        <Section title="Route Assignments" icon={<MapPin className="w-4 h-4" />}>
          {bus.routeAssignments.length > 0 ? (
            bus.routeAssignments.map(r => (
              <div key={r.routeId} className="py-2 border-b border-border/50 last:border-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="inline-block px-2 py-0.5 bg-primary/10 text-primary border border-primary/20 text-xs rounded font-mono">
                    {r.routeNumber}
                  </span>
                  <span className="text-sm font-medium text-foreground">{r.routeName}</span>
                </div>
                <p className="text-xs text-muted-foreground">
                  {r.origin} → {r.destination}
                </p>
                <p className="text-xs text-muted-foreground/70 mt-0.5">Permit: <span className="font-mono">{r.permitNumber}</span></p>
              </div>
            ))
          ) : (
            <p className="text-sm text-muted-foreground/70 italic py-2">No route assigned</p>
          )}
        </Section>
      </div>
