    ...
    <div className="space-y-6">
      ...
      <div className={`${cfg.bg} border border-border rounded-xl p-6`}>
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-muted-foreground uppercase tracking-wide font-medium">Current Status</p>
            <p className="text-2xl font-bold text-foreground">{cfg.label}</p>
            <p className="text-xs text-muted-foreground mt-0.5">Last updated: {formatDateTime(trip.updatedAt)}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl p-6">
        <h3 className="text-base font-semibold text-foreground mb-4">Timing Analysis</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {/* Departure analysis */}
          <div className="bg-muted rounded-lg p-4">
            <p className="text-xs text-muted-foreground uppercase font-semibold tracking-wide mb-3">Departure</p>
                ...
                <div className="flex justify-between items-center border-t border-border pt-2">
                  ...
                  <span
                    className={`text-sm font-semibold ${
                      depDelay > 0 ? 'text-destructive' : depDelay < 0 ? 'text-success' : 'text-muted-foreground'
                    }`}
                  >
          ...
          {/* Arrival analysis */}
          <div className="bg-muted rounded-lg p-4">
            <p className="text-xs text-muted-foreground uppercase font-semibold tracking-wide mb-3">Arrival</p>
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-sm text-muted-foreground">Scheduled</span>
              ...
              {arrDelay !== null && (
                <div className="flex justify-between items-center border-t border-border pt-2">
                  ...
                  <span
                    className={`text-sm font-semibold ${
                      arrDelay > 0 ? 'text-destructive' : arrDelay < 0 ? 'text-success' : 'text-muted-foreground'
                    }`}
                  >
                ...
                <div key={idx} className="flex gap-4 pb-4 last:pb-0">
                  ...
                  <div className="flex flex-col items-center">
                    ...
                    {idx < history.length - 1 && (
                      <div className="w-0.5 flex-1 bg-secondary my-1 min-h-[20px]" />
                    )}
                  </div>
                  {/* Event content */}
                  <div className="flex-1 min-w-0 pb-2">
                    <p className="text-sm font-semibold text-foreground">{evtCfg.label}</p>
      ...
      {(trip.notes || trip.cancellationReason) && (
        <div className="bg-card border border-border rounded-xl p-5 space-y-3">
          ...
          {trip.cancellationReason && (
            <div>
              <p className="text-xs font-semibold text-destructive/80 uppercase tracking-wide mb-1">
                Cancellation Reason
              </p>
              <p className="text-sm text-destructive">{trip.cancellationReason}</p>
            </div>
          )}
        </div>
      )}
