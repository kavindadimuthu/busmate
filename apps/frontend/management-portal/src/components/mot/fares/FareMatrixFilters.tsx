    ...
    <div className="bg-card border border-border rounded-2xl shadow-sm overflow-hidden">
      ...
      <div className="flex flex-wrap items-end gap-4 p-4 border-b border-border/50">
        ...
        <div className="flex-1 min-w-[200px]">
          <label className="block text-xs font-medium text-muted-foreground mb-1.5">Search by fare amount</label>
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-9 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-primary/40"
            />
            {searchFare && (
              <button
                onClick={() => onSearchFareChange('')}
                className="absolute right-2.5 top-1/2 -translate-y-1/2 p-0.5 rounded-full text-muted-foreground/70 hover:text-muted-foreground hover:bg-muted"
              >
        ...
        <div className="w-[130px]">
          <label className="block text-xs font-medium text-muted-foreground mb-1.5">Stage from</label>
          <input
            type="number"
            min={1}
            max={stageTo}
            ...
            className="w-full px-3 py-2 border border-border rounded-lg text-sm tabular-nums focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-primary/40"
          />
        </div>

        {/* Stage To */}
        <div className="w-[130px]">
          <label className="block text-xs font-medium text-muted-foreground mb-1.5">Stage to</label>
          <input
            type="number"
            min={stageFrom}
            max={maxStages}
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground py-2">
          <span>
            Showing stages <span className="font-semibold text-foreground/80">{stageFrom}</span> – <span className="font-semibold text-foreground/80">{stageTo}</span> of {maxStages}
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 px-4 py-3">
              ...
              <button
                ...
                className={[
                  ...
                  isSelected
                    ...
                    : 'bg-muted text-muted-foreground/70 border-border hover:bg-muted',
                ].join(' ')}
              >
        ...
        {hasActiveFilters && (
          <button
            onClick={onClearAll}
            className="ml-auto text-xs font-medium text-destructive hover:text-destructive hover:underline transition-colors"
          >
            Clear all filters
          </button>
        )}
