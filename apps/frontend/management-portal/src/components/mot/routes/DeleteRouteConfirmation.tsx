              ...
              <div className="space-y-6">
                  ...
                  <div className="bg-muted rounded-lg p-4">
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <span className="ml-2 text-sm text-foreground">{routeGroup.name || 'N/A'}</span>
                      </div>
                      <div>
                        <span className="text-sm font-medium text-foreground/80">ID:</span>
                        <span className="ml-2 text-sm font-mono text-muted-foreground">{routeGroup.id}</span>
                      </div>
                      {routeGroup.description && (
                        <div>
                          <span className="text-sm font-medium text-foreground/80">Description:</span>
                          <span className="ml-2 text-sm text-foreground">{routeGroup.description}</span>
                        </div>
                      )}
                      <div className="flex items-center">
                        <span className="text-sm font-medium text-foreground/80">Routes:</span>
                        <div className="flex items-center ml-2">
                          <Route className="h-4 w-4 text-muted-foreground/70 mr-1" />
                ...
                {route && (
                  <div className="bg-muted rounded-lg p-4">
                    ...
                    <div className="space-y-3">
                      <div>
                        <span className="text-sm font-medium text-foreground/80">Name:</span>
                        <span className="ml-2 text-sm text-foreground">{route.name || 'N/A'}</span>
                      </div>
                      <div>
                        <span className="text-sm font-medium text-foreground/80">ID:</span>
                        <span className="ml-2 text-sm font-mono text-muted-foreground">{route.id}</span>
                      </div>
                      {route.description && (
                        <div>
                          <span className="text-sm font-medium text-foreground/80">Description:</span>
                          <span className="ml-2 text-sm text-foreground">{route.description}</span>
                        </div>
                      )}
                      <div>
                        <span className="text-sm font-medium text-foreground/80">Route Group:</span>
                        <span className="ml-2 text-sm text-foreground">{route.routeGroupName || 'N/A'}</span>
                      </div>
                      <div>
                        <span className="text-sm font-medium text-foreground/80">Direction:</span>
                        <span className="ml-2 text-sm text-foreground">{route.direction || 'N/A'}</span>
                      </div>
                      <div>
                        <span className="text-sm font-medium text-foreground/80">Route:</span>
                        <span className="ml-2 text-sm text-foreground">
                      ...
                      {route.distanceKm && (
                        <div>
                          <span className="text-sm font-medium text-foreground/80">Distance:</span>
                          <span className="ml-2 text-sm text-foreground">{route.distanceKm.toFixed(1)} km</span>
                        </div>
                      )}
                      {route.createdAt && (
                        <div>
                ...
                {routeGroup?.routes && routeGroup.routes.length > 0 && (
                  <div className="bg-warning/10 border border-orange-200 rounded-lg p-4">
                    <h4 className="text-sm font-medium text-warning mb-3">
