  ...
  if (scheduleStops.length === 0) {
    return (
      <div className="text-center py-12">
        <MapPin className="mx-auto h-12 w-12 text-muted-foreground/70 mb-4" />
        <h3 className="text-lg font-medium text-foreground mb-2">No Schedule Stops</h3>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h3 className="text-lg font-medium text-foreground">Schedule Stops</h3>
          <p className="mt-1 text-sm text-muted-foreground">
            {scheduleStops.length} stops configured for this schedule
          </p>
        </div>
        <div className="flex items-center space-x-3  border border-primary/20 rounded-lg px-4 py-2">
          <input
            type="checkbox"
          ...
          />
          <label htmlFor="showOnlyWithTimings" className="flex-1 cursor-pointer text-sm text-foreground/80">
          ...
          {scheduleStops
            ...
            .map((stop, index) => {
              ...
              return (
                ...
                >
                  {/* Stop Number Badge */}
                  <div className="shrink-0 w-10 h-10 bg-primary text-white rounded-full flex items-center justify-center font-medium text-sm">
                  ...
                  <div className="shrink-0 ml-4 text-center min-w-[70px]">
                    <div className="text-xs text-muted-foreground mb-1">Dwell</div>
      ...
      <div className="bg-primary/10 rounded-lg p-4">
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
          <div className="text-center">
            <div className="text-2xl font-bold text-primary">{scheduleStops.length}</div>
            <div className="text-primary">Total Stops</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-primary">
              {formatTime(scheduleStops[0]?.departureTime)}
            </div>
            <div className="text-primary">Departure from start</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-primary">
              {formatTime(scheduleStops[scheduleStops.length - 1]?.arrivalTime)}
            </div>
            <div className="text-primary">Arrival at end</div>
          </div>
        </div>
      </div>
    </div>
