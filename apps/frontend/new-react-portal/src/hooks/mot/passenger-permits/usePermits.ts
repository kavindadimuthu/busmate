...
import type { PermitFilters } from '@/components/mot/passenger-permits/PermitsFilterBar';
...
export function usePermits() {
  ...
  const handleView = useCallback(
    (permit: PassengerServicePermitResponse) => router.push(`/mot/passenger-permits/${permit.id}`),
    [router],
  );

  const handleEdit = useCallback(
    (permit: PassengerServicePermitResponse) => router.push(`/mot/passenger-permits/${permit.id}/edit`),
    [router],
  );

  const handleAssignBus = useCallback(
    (permit: PassengerServicePermitResponse) => router.push(`/mot/passenger-permits/${permit.id}/assign-bus`),
    [router],
  );
