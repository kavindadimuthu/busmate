...
export function NotificationDetailPanel({ notification, onBack }: NotificationDetailPanelProps) {
  ...
  const typeConfig: Record<string, { cls: string; icon: React.ReactNode; label: string }> = {
    info: { cls: 'bg-primary/15 text-primary', icon: <Info className="h-5 w-5" />, label: 'Information' },
    warning: { cls: 'bg-warning/15 text-warning', icon: <AlertTriangle className="h-5 w-5" />, label: 'Warning' },
    critical: { cls: 'bg-destructive/15 text-destructive', icon: <XCircle className="h-5 w-5" />, label: 'Critical' },
    success: { cls: 'bg-success/15 text-success', icon: <CheckCircle className="h-5 w-5" />, label: 'Success' },
    maintenance: { cls: 'bg-[hsl(var(--purple-100))] text-[hsl(var(--purple-800))]', icon: <Wrench className="h-5 w-5" />, label: 'Maintenance' },
    error: { cls: 'bg-destructive/15 text-destructive', icon: <XCircle className="h-5 w-5" />, label: 'Error' },
  };

  const statusConfig: Record<string, { cls: string; icon: React.ReactNode; label: string }> = {
    sent: { cls: 'bg-success/10 text-success border border-success/20', icon: <Send className="h-3.5 w-3.5" />, label: 'Sent' },
    scheduled: { cls: 'bg-warning/10 text-warning border border-warning/20', icon: <Clock className="h-3.5 w-3.5" />, label: 'Scheduled' },
    draft: { cls: 'bg-muted text-muted-foreground border border-border', icon: <FileEdit className="h-3.5 w-3.5" />, label: 'Draft' },
    failed: { cls: 'bg-destructive/10 text-destructive border border-destructive/20', icon: <XCircle className="h-3.5 w-3.5" />, label: 'Failed' },
  };

  const priorityColors: Record<string, string> = {
    critical: 'bg-destructive text-white',
    high: 'bg-warning/15 text-orange-700 border border-orange-200',
  ...
  return (
    <div className="mx-auto space-y-6">
      ...
      <div className="bg-card rounded-xl border border-border shadow-sm p-6">
        <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide mb-3">Message</h3>
        <p className="text-foreground/80 leading-relaxed whitespace-pre-wrap">{n.body}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-card rounded-xl border border-border shadow-sm p-6">
          ...
          <div className="space-y-4">
            <DetailRow
              icon={<Calendar className="h-4 w-4 text-muted-foreground/70" />}
              label="Created"
            ...
            {n.sentAt && (
              <DetailRow
                icon={<Send className="h-4 w-4 text-muted-foreground/70" />}
                label="Sent"
            ...
            />
            <DetailRow
              icon={<Users className="h-4 w-4 text-muted-foreground/70" />}
              label="Audience"
              value={audienceLabel}
            />
            <DetailRow
            ...
            />
            <DetailRow
              icon={<Bell className="h-4 w-4 text-muted-foreground/70" />}
              label="ID"
              value={n.id}
        ...
        <div className="bg-card rounded-xl border border-border shadow-sm p-6">
          <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide mb-4">Delivery</h3>
          {n.status === 'sent' ? (
            <div className="space-y-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-foreground">{n.totalRecipients.toLocaleString()}</p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-foreground">{n.readCount.toLocaleString()}</p>
                  <p className="text-xs text-muted-foreground">Read / opened</p>
                </div>
              </div>
                ...
                <div className="h-2.5 bg-muted rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-500 ${
                      readRate >= 80 ? 'bg-success' :
                      readRate >= 50 ? 'bg-primary/80' :
                      readRate >= 25 ? 'bg-warning' :
                      'bg-destructive'
                    }`}
