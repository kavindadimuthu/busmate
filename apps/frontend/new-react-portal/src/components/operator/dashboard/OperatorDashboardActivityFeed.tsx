...
const ACTOR_ICON: Record<ActivityEntry['actorType'], React.ReactNode> = {
  ...
  conductor: <UserCheck className="h-3.5 w-3.5" />,
...
const ACTOR_STYLE: Record<ActivityEntry['actorType'], string> = {
  ...
  conductor: 'bg-[hsl(var(--purple-100))] text-[hsl(var(--purple-600))]',
...
interface OperatorDashboardActivityFeedProps {
  activity: ActivityEntry[];
  loading?: boolean;
}

export function OperatorDashboardActivityFeed({ activity, loading = false }: OperatorDashboardActivityFeedProps) {
  if (loading) {
    return (
    ...
    <div className="bg-card rounded-xl border border-border p-6 flex flex-col h-full">
      ...
      <div className="space-y-1 flex-1 overflow-auto">
        {activity.map((entry) => (
          ...
          >
            {/* Actor icon */}
            <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${ACTOR_STYLE[entry.actorType]}`}>
