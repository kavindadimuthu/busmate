      ...
      <div className="px-6 py-5 border-b border-border/50">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <h2 className="text-lg font-semibold text-foreground truncate">{amendment.title}</h2>
            <p className="text-sm text-muted-foreground mt-1 font-mono">{amendment.referenceNumber}</p>
          </div>
          <span
        ...
        {amendment.description && (
          <p className="text-sm text-muted-foreground mt-3 leading-relaxed">{amendment.description}</p>
        )}
      </div>
      ...
      {firstEntry && lastEntry && (
        <div className="px-6 py-4 border-t border-border/50 bg-muted/50">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {PERMIT_TYPES.map((pt) => (
              <div key={pt} className="bg-card rounded-lg border border-border p-3">
                <p className="text-xs font-medium text-muted-foreground">{PERMIT_TYPE_LABELS[pt]}</p>
                <p className="text-sm font-semibold text-foreground mt-1">
                  {firstEntry.fares[pt].toFixed(2)} – {lastEntry.fares[pt].toFixed(2)}
                </p>
                <p className="text-[10px] text-muted-foreground/70 mt-0.5">Stage 1 – {amendment.maxStages}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {amendment.remarks && (
        <div className="px-6 py-4 border-t border-border/50">
          <div className="flex items-start gap-2">
            ...
            <p className="text-sm text-muted-foreground">{amendment.remarks}</p>
          </div>
        </div>
      )}
    </div>
