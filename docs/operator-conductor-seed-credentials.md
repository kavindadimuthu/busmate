...
Every account here is fully aligned with the unified operator lifecycle: each operator's core-service `Operator` row was created through the real `/internal/operators` sync endpoint and carries a `userId` link back to its account — not raw seed SQL pretending to be linked.
