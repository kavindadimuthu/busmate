...
The first six are `GPS_TRACKER`s with a static bus assignment. The last is a `CONDUCTOR_APP` device
with **no** assignment — it reports whichever bus it's currently on by sending a `tripId` hint with
every fix (see `IngestService.resolveBusAndTrip`); this is what conductor-mobile uses (see
`src/services/telemetry/locationReporting.ts` in that app) and what `EXPO_PUBLIC_CONDUCTOR_DEVICE_TOKEN`
defaults to in dev.
