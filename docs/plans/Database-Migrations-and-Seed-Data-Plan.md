...
`schema.sql`, and `db/migration/V00x` files, and the older `docs/database-*-guide.md` docs, predate
the self-hosted auth migration and are **stale**. This plan is grounded in the *current* entity
models, not those files, and supersedes them. They are removed in Phase 5.
...
1. Generate `V001__baseline.sql` from the **current** schema: bring up a clean DB with the current
   entities (`ddl-auto: update` one last time against an empty database), then
   `pg_dump --schema-only --no-owner --no-privileges`. Scrub any provider-specific artifacts so the
   file is plain Postgres.
2. Keep `spring.flyway.baseline-on-migrate: true` and `baseline-version: 1` so existing prod
   (Supabase) and long-lived dev databases *adopt* the baseline instead of trying to recreate
   existing tables.
3. Flip `ddl-auto` from `update` to `validate`. **Startup now fails loudly if entities and the
   baseline disagree** — fix the drift that `update` was silently hiding. This convergence of dev
   and prod schema is the primary payoff of the whole effort.
